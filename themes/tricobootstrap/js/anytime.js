...
(function($)
{
  ...
  var __pickers = [];
    ...
    // trico edit 2012-11-27 ah - adding new method to allow currently selected
    //    val to be set by the script
  	$.fn.AnyTime_setSelected = function( options )
    {
  		return this.each( function(i) { AnyTime.setSelected( this.id, options ); } );
    }
...
AnyTime.picker = function( id, options )
{
    ...
    __pickers[id] =
    {
        ...
        // trico edit 2014.03.05 ah - migrating customizations
        blackouts: null,     // blackout dates (should be first hour in day)
      ...
      this.id = 'AnyTime--'+id.replace(/[^-_.A-Za-z0-9]/g,'--AnyTime--');
            ...
            if ( options.blackouts )
            {
                ...
                var bodate;
                for ( i = 0 ; i < options.blackouts.length ; i++ )
                {
                    bodate = options.blackouts[i];
                    if ( typeof bodate.getTime == 'function' ) {
                        this.blackouts.push( bodate.getTime());
                    } else {
                        botmp = this.conv.parse(bodate.toString());
                        botmp.setHours(12,0,0,0); //for making comparisons
                        this.blackouts.push( botmp.getTime() );
                    }
                }
            }
      ...
      var _this = this;
        ...
        set: function(newTime)
        {
            ...
            if ( this.earliest && ( t < this.earliest ) )
              ...
              this.time = newTime;
            // trico edit 2012-11-27 ah - 
            // note: it may seem like a good idea here to also go through
            // blackout days.   However, we wouldn't know whether to bump the
            // date forward or back.  So instead blackoutdays are depicted in
            // UI, but can be selected, and must therefore be checked on form
            // validation.
		},
		  
		//---------------------------------------------------------------------
        // trico edit 2012-11-27 ah - new method allows currently selected val
        //   to be set by calling script
        //
		//  .setSelected() basically a wrapper for set.
		//---------------------------------------------------------------------
		
		setSelected: function(newTime)
		{
          newTime = this.conv.parse( newTime.toString() );
          this.set(newTime);
          this.upd(null);
        },
        ...
        {
          // trico EDIT 2012-11-08 ah - this wasn't working right.
          newTime = this.conv.parse( newTime.toString() );
      ...
      this.pos(event);
                      ...
                      if ( dow - _this.fDOW < 7 )
                      {
                          ...
                          else
                          {
                            ...
                            td.AnyTime_current( i == current,
                                    ((!_this.earliest)||(cmpHi.getTime()>=_this.earliest)) &&
					        		((!_this.latest)||(cmpLo.getTime()<=_this.latest)) &&
                                    ((!_this.blackouts)||(jQuery.inArray(cmpLo.getTime(), _this.blackouts) == -1)) );
                            cmpLo.setDate(i+1);
                            cmpHi.setDate(i+1);
                          }
                      }
        ...
        cmpHi.setHours( this.time.getHours(), 9 );
            ...
            $('#'+this.id+' .AnyTime-min-one-btn:not(.AnyTime-min-one-btn-empty)').each(
              function()
              {
                // trico EDIT 2012-11-06 ah -
                // we don't want any ones digit in the minutes for booking.
		        $(this).AnyTime_current( this.innerHTML == ones, this.innerHTML == 0 );
		        		//((!_this.earliest)||(cmpHi.getTime()>=_this.earliest)) &&
		        		//((!_this.latest)||(cmpLo.getTime()<=_this.latest)) );
...
//  AnyTime.setSelected()
//
//  Updates the selected date/time for the picker attached to a specified
//  text field.
// trico edit 2012-11-27 ah - adding new method to allow currently selected
//   val to be set by calling script
//=============================================================================

AnyTime.setSelected = function( id, newTime )
{
  __pickers[id].setSelected(newTime)
};
