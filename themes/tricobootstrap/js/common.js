...
$(document).ready(function() {
  ...
  if(url.indexOf('?' + 'print' + '=') != -1  || url.indexOf('&' + 'print' + '=') != -1) {
    ...
    $(document).ajaxStop(function() {
      // trico edit
      // if Print = false do not display print pop up 
      // this was added for the Zotero link that mimics Print but is not 
      // called print
  ...
  // Save lightbox for Primo and DigArch
  $('.save-record').click(function() {
    var params = extractClassParams(this);
    return Lightbox.get(params['controller'], 'Save', {id:$(this).attr('id')});
  });
  ...
  $('.sms-record').click(function() {
    var params = extractClassParams(this);
    return Lightbox.get(params['controller'], 'SMS', {id:$(this).attr('id')});
  });
  // Mail lightbox
  $('.mail-record').click(function() {
    var params = extractClassParams(this);
    return Lightbox.get(params['controller'], 'Email', {id:$(this).attr('id')});
  });
  ...
  // trico edit 2014.04.29 ah - migrations from 1.3 
  $('a.smsRecord').click(function() {
      //var id = this.id.substr('smsRecord'.length);
      var module = 'Record';
      if ($(this).hasClass('smsSummon')) {
          module = 'Summon';
      } else if ($(this).hasClass('smsArticles')) {
          module = 'Articles';
      } else if ($(this).hasClass('smsWorldCat')) {
          module = 'WorldCat';
      }
      return Lightbox.get(module, 'SMS');
  });
  $('a.mailRecord').click(function() {
      var module = 'Record';
      if ($(this).hasClass('mailSummon')) {
          module = 'Summon';
      } else if ($(this).hasClass('mailArticles')) {
          module = 'Articles';
      } else if ($(this).hasClass('mailWorldCat')) {
          module = 'WorldCat';
      }
      return Lightbox.get(module, 'Email');
  });
  // assign click event to "Need Help" link
  $('a.needHelp').click(function() {
      return Lightbox.get('Search', 'Help');
  });

  // assign click event to login link
  $('a.login').click(function() {
      return Lightbox.get('MyResearch', 'Login');
  });

  // trico edit 2014.05.06 ah - add logout lightbox
  $('a#logout').click(function() {
    Lightbox.addCloseAction(function() {
      document.location.href = path;
    });
    return Lightbox.get('MyResearch', 'Logout');
  });
});
